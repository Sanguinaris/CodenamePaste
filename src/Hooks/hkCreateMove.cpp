...
bool __fastcall hkCreateMove(void* ecx,
  ...
  auto IEngineClient = GetInterfaceWrap(
      static_cast<CodeNamePaste::Managers::Hooks::HookingManager*>(hkManager)
          ->ifaceMgr,
      Interfaces::IClientEngine, "VEngineClient");
  if (static_cast<CodeNamePaste::Managers::Hooks::HookingManager*>(hkManager)
          ->ClientMode->GetOriginal()
          ->CreateMove(sequence, userCMD))
...

  {
    ...
    for (const auto& clbk :
         static_cast<CodeNamePaste::Managers::Hooks::HookingManager*>(hkManager)
             ->funcCallbacks
                 [CodeNamePaste::Managers::Hooks::HookNames::CreateMove]) {
      clbk(userCMD);
    }
  }
  return false;
