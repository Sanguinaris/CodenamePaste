...
void NetVarManager::DoInit()
{
	...
	for (auto client = GetInterfaceWrap(ifaceMgr, IBaseClientDLL, "VClient")->GetAllClasses()
...
const CodeNamePaste::Managers::AutoNum NetVarManager::GetProp(const std::string& tableName, const std::string& propName, RecvProp** prop) const
{
...
const CodeNamePaste::Managers::AutoNum NetVarManager::GetProp(const RecvTable& recvTable, const std::string& propName, RecvProp** prop) const
{
	AutoNum addOffy = 0;
